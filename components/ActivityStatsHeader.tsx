...
export function ActivityStatsHeader({ activity }: { activity: CachedActivity }) {
  return (
    <div className="flex flex-col gap-5">
      <div className="flex flex-col gap-1">
        <h1 className="text-2xl font-semibold tracking-tight">{activity.name}</h1>
        <p className="text-sm" style={{ color: "var(--text-secondary)" }}>
          {formatDateTr(activity.start_date_local)} · {formatTimeTr(activity.start_date_local)}
        </p>
      </div>

      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <BigStat label="Mesafe" value={formatDistanceKm(activity.distance)} unit="km" />
        <BigStat label="Süre" value={formatDurationShort(activity.moving_time)} />
        <BigStat label="Kalori" value={formatCalories(activity.calories)} unit="kcal" accent />
        <BigStat
          label="Ort. Pace"
          value={formatPace(activity.distance, activity.moving_time).replace(" /km", "")}
          unit="/km"
        />
      </div>

      <div className="card grid grid-cols-2 sm:grid-cols-4 divide-x" style={{ borderColor: "var(--border)" }}>
        <SmallStat
          label="Ort. Nabız"
        ...
        <SmallStat
          label="Maks. Nabız"
          value={activity.max_heartrate ? `${Math.round(activity.max_heartrate)}` : "-"}
          unit="bpm"
        />
        <SmallStat
      ...
      <div className="card grid grid-cols-2 sm:grid-cols-4 divide-x" style={{ borderColor: "var(--border)" }}>
        ...
        <SmallStat
          label="Maks. Hız"
          value={activity.max_speed ? (activity.max_speed * 3.6).toFixed(1) : "-"}
          unit="km/s"
        />
        <SmallStat
          label="Efor Skoru"
